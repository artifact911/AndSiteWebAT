...
@Getter
public class ContactUsFooterUs implements FormData, CUFooterLocatorsUs, Spamable {
    private CUFooterElementsUs el = new CUFooterElementsUs();
...
//        Assert.assertEquals(el.getInputPhone().getAttribute("placeholder"), "Phone");
    ...
    public void sendButtonHovered() {
        Assert.assertEquals(el.getButtonSend().getCssValue("text-decoration"), "none solid rgb(0, 0, 0)");
        Assert.assertEquals(el.getButtonSend().hover().getCssValue("animation"), "none 0s ease 0s 1 normal none running");
...
//        Assert.assertNotEquals(el.getLinkSkype().hover().getCssValue("background"), "rgba(255, 255, 255, 0.1) none repeat scroll 0% 0% / auto padding-box border-box");
        System.out.println(el.getLinkSkype().hover().getCssValue("background")); //FIXME
    ...
    public void linkPhoneMail(Country country) {
        System.out.println(el.getLinkPhone());
    ...
    public void enterNotCooperateMail() {
        ...
        //хинт отсутствует. аск фон появится
        Assert.assertTrue(el.getAskPhone().getAttribute("class").contains("field_error"));
        Assert.assertEquals(el.getAskPhone().getAttribute("data-error-message"), "Please enter a valid information");
    }
    ...
    public void emptyInputRequest() {
        ...
        el.getButtonSend().click();
        Assert.assertEquals(el.getAskPhone().getAttribute("data-error-message"), TEXT_ASK_PHONE_MAIL);
        Assert.assertEquals(el.getAskPhone().getAttribute("class"), "control field_error");
        Assert.assertEquals(el.getAskMail().getAttribute("data-error-message"), TEXT_ASK_PHONE_MAIL);
    ...
    public void recordIsPresentDb(String name) {
        ...
        final String QUERY = "SELECT * FROM `form_data` WHERE `email` = 'qaands+3@gmail.com' AND `name`='" + name + "'  AND `date` BETWEEN \n" +
    ...
    public String goodReq(String mail, String phone) {
        ...
        if (Spamable.CHECK_BOX) {Logger.getLogger().log("not applicable"); throw new IllegalArgumentException("not applicable");}
        el.getButtonSend().click();
        el.getButtonCloseSuccessWindow().click();
        return uniqName;
    }

...
public class CalcForm implements FormData, CalcFormLocators, Spamable, FormPopUp {
    //этот класс используется для проверки "Please, leave your contact details to receive the quote" в блоке Know the real price of the project
    //отличается от RequestForm 2-мя строками в интерфейсе, но объединять нецелесообразно ввиду удвоения строк кода на локаторы и элементы
    private final CalcFormElements el = new CalcFormElements();
    ...
    public CalcForm(String title, Success success) {
    ...
    public void checkTitle() {
        Assert.assertEquals(el.getTitleWindowReq().getAttribute("innerText"), title);
...
//        CalcFormLocators.doBefore1();
//        //временно
//        Warp.clickSimpleIfPresent("//div[contains(@class, 'popup-calc-gifts__content')]//div[contains(@class, 'close')]");
//    }
    ...
    private void emptyInputRequest() {
        ...
        Assert.assertEquals(el.getAskPhone().getAttribute("class"), "forms-module--error--3R6sz");
        Assert.assertEquals(el.getAskMail().getAttribute("class"), "forms-module--error--3R6sz");
    }

...
public class RequestForm implements FormData, RequestFormLocators, Spamable, FormPopUp {
    ...
    private Country country = Country.BY;//значение по умолчанию
    ...
    public void checkTitle() {
        $(el.getTitleWindowReq()).should(Condition.appear);
    ...
    private void emptyInputRequest() {
        ...
        Selenide.Wait().until(ExpectedConditions.elementToBeClickable(el.getButtonSendReq()));
        el.getButtonSendReq().click();
        Selenide.Wait().until(ExpectedConditions.visibilityOf(el.getAskPhone()));

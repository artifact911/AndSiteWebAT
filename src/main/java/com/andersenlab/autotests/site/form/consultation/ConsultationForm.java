...
@Getter
public class ConsultationForm implements FormData, ConsultationFormLocators, Spamable {
    //класс для проверки free consultation перед футером. очень похоже на RequestForm, но разная база для локаторов,
    ...
    private final ConsultationFormElements el = new ConsultationFormElements();
    ...
    public ConsultationForm() {
        this.title = "Contact us for a free consultation";
        this.success = new SuccessPoor();
    }

    public ConsultationForm(String title) {
        this.title = title;
        this.success = new SuccessPoor();
    }

    public ConsultationForm(String title, String subtitle) {
        this.title = title;
        this.subtitle = subtitle;
        this.success = new SuccessPoor();
    }

    public ConsultationForm(String title, Success success) {
        this.title = title;
    ...
    public void checkTitle() {
        Assert.assertEquals(el.getTitle().should(Condition.appear).getText(), title);
    ...
    @Step
    public void checkCAPTCHALinks() {
        checkPrivacyPolicy();
        checkTermsOfService();
    }

    private void checkPrivacyPolicy() {
        Warp.checkLinkNewTab(el.getPrivacyPolicy(), Google.expectedPrivacyLink);
    }

    private void checkTermsOfService() {
        Warp.checkLinkNewTab(el.getTermsOfService(), Google.expectedTermsLink);
    }

    @Step
    public void checkFormFields() {
        String name = "Henry";
        String mail = "real@mail";
        String phone = "1111111";
        String comment = "Hi! I'd like to talk to someone. I have an idea to implement and I want to discuss the realization.";
        checkNameField(name);
        checkEmailField(mail);
        checkPhoneField(phone);
        checkCommentsField(comment);
    }

    private void checkNameField(String name) {
        el.getInputName().sendKeys(name);
        Assert.assertEquals(el.getInputName().getAttribute("value"), name);
    }

    private void checkEmailField(String mail) {
        el.getInputMail().sendKeys(mail);
        Assert.assertEquals(el.getInputMail().getAttribute("value"), mail);
    }

    private void checkPhoneField(String phone) {
        el.getInputPhone().sendKeys(phone);
        Assert.assertEquals(el.getInputPhone().getAttribute("value"), el.getInputPhoneMask() + phone);
    }

    private void checkCommentsField(String comment) {
        el.getInputComment().sendKeys(comment);
        Assert.assertEquals(el.getInputComment().getText(), comment);
    }
    ...
    private void checkInfoPopup() {
        ...
        el.getHintTextForNDA().shouldBe(Condition.visible);
